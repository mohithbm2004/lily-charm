        ...
        <>
          ...
          >
            <div className="flex items-center justify-between px-4 sm:px-6 h-16 sm:h-20 border-b border-[var(--color-line)] shrink-0">
              ...
              <button onClick={closeCart} aria-label="Close cart" className="p-1 hover:bg-black/5 rounded">
            ...
            <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6">
              {items.length === 0 && (
                <div className="text-center py-12 text-[var(--color-ink-soft)] space-y-3">
                  ...
                  <Link to="/shop" onClick={closeCart} className="btn-outline text-xs inline-block">
                    Explore Shop
                  </Link>
                </div>
              )}
              {items.map((item) => (
                <div key={item.id} className="flex gap-3 sm:gap-4 border-b border-[var(--color-line)]/40 pb-4">
                  <img src={item.image} alt={item.title} className="w-16 h-20 sm:w-20 sm:h-24 object-cover shrink-0 border border-[var(--color-line)] bg-[var(--color-card-bg)]" />
                  <div className="flex-1 min-w-0 flex flex-col justify-between">
                    ...
                    <div className="flex items-center justify-between gap-2 mt-2">
                      <div className="flex items-center border border-[var(--color-line)] bg-[var(--color-card-bg)]">
                        <button onClick={() => setQty(item.id, item.qty - 1)} className="w-7 h-7 flex items-center justify-center hover:bg-black/5" aria-label="Decrease"><Minus size={11} /></button>
                        <span className="w-7 text-center text-xs font-bold font-mono">{item.qty}</span>
                        <button onClick={() => setQty(item.id, item.qty + 1)} className="w-7 h-7 flex items-center justify-center hover:bg-black/5" aria-label="Increase"><Plus size={11} /></button>
                      </div>
                  ...
                  <form onSubmit={handleApplyCoupon} className="space-y-1">
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 border border-[var(--color-line)] bg-transparent px-3 py-2 text-xs uppercase focus:outline-none focus:border-[var(--color-primary)] font-mono min-w-0"
                      />
